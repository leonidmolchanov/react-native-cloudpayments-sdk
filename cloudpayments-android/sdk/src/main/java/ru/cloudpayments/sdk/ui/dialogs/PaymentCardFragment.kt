...
internal class PaymentCardFragment :
		...
		binding.editCardNumber.addTextChangedListener(object : TextWatcherAdapter() {
			override fun afterTextChanged(s: Editable?) {
				...
				viewModel.getBinInfo(cardNumber,
									 ...
									 sdkConfig?.terminalConfiguration?.isAllowedNotSanctionedCards ?: true,
									 sdkConfig?.terminalConfiguration?.isQiwi ?: false)
	...
	private fun isValid(): Boolean {
		...
		val cardCvvIsValid = Card.isValidCvv(binding.editCardCvv.text.toString(), binding.tilCardCvv.isVisible)
...
//		errorMode(!cardCvvIsValid, binding.editCardCvv)
	...
	private fun updateForm(cardNumber: String) {
		...
		if (cardNumber.length < 4) {
			if (sdkConfig?.terminalConfiguration?.isCvvRequired == true) {
				showCvv()
			} else {
				hideCvv()
			}
