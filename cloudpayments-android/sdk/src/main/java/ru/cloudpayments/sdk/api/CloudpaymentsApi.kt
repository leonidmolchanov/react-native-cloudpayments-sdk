...
class CloudpaymentsApi @Inject constructor(private val apiService: CloudpaymentsApiService) {
	...
	fun getTPayQrLink(requestBody: QrPayLinkBody): Single<CloudpaymentsGetQrPayLinkResponse> {
		return apiService.getTPayQrLink(requestBody)
			.subscribeOn(Schedulers.io())
	}

	fun getSberPayQrLink(requestBody: QrPayLinkBody): Single<CloudpaymentsGetQrPayLinkResponse> {
		return apiService.getSberPayQrLink(requestBody)
			.subscribeOn(Schedulers.io())
	}
	...
	fun getMirPayLink(requestBody: MirPayLinkBody): Single<CloudPaymentsGetMirPayLinkResponse> {
		return apiService.getMirPayLink(requestBody)
			.subscribeOn(Schedulers.io())
	}

	fun getMirPayCryptogram(guid: String): Single<CloudPaymentsGetMirPayCryptogramResponse> {

		val body = GetMirPayCryptogramBody(guid = guid)

		return apiService.getMirPayCryptogram(body)
			.subscribeOn(Schedulers.io())
	}
